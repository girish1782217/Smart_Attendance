...
| `system_settings` | key (PK), value, updated_at | minimal key/value store; currently holds only `LOW_ATTENDANCE_THRESHOLD` (Assumption A-3). Falls back to `.env`'s `LOW_ATTENDANCE_DEFAULT_THRESHOLD` when unset. |
| `notifications` | user_id, type, title, message, related_entity_type, related_entity_id, is_read, created_at, read_at | in-app only (Assumption A-7); `type` is `CORRECTION_REQUESTED`\|`CORRECTION_APPROVED`\|`CORRECTION_REJECTED`\|`LOW_ATTENDANCE_WARNING` |
